...
def main():
    ...
    print("Generating synthetic data...")
    X, y = make_synthetic_blob_data(n_samples=n_samples, n_outliers=n_outliers, random_state=42)
    ...
    plt.scatter(X_test[anomaly_mask == 0, 0], X_test[anomaly_mask == 0, 1], c='blue', label='Normal', alpha=0.5)
    plt.scatter(X_test[anomaly_mask == 1, 0], X_test[anomaly_mask == 1, 1], c='red', label='Anomaly', alpha=0.5)
    ...
    # Plot normal points
    plt.scatter(X_test[y_test == 0, 0], X_test[y_test == 0, 1], c='blue', label='Normal', alpha=0.5)
    
    # Plot anomalies
    plt.scatter(X_test[y_test == 1, 0], X_test[y_test == 1, 1], c='red', label='Anomaly', alpha=0.5)
