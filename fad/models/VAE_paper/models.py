...
from .losses import make_mse_kl, make_mse, make_kl

# number of integer bits for each bit width
QUANT_INT = {0: 0, 2: 1, 4: 2, 6: 2, 8: 3, 10: 3, 12: 4, 14: 4, 16: 6}
...
def model_set_weights(model, load_model, quant_size):
    ...
    bp_model = tf.keras.models.model_from_json(
        config,
        custom_objects={
    ...
    if quant_size != 0:
        for i, _ in enumerate(model.layers[1].layers):
            if i < 2:
                continue
            model.layers[1].layers[i].set_weights(
                bp_model.layers[1].layers[i - 1].get_weights()
            )
    else:
        for i, _ in enumerate(model.layers[1].layers):
            model.layers[1].layers[i].set_weights(
                bp_model.layers[1].layers[i].get_weights()
            )
    # set weights for decoder
    for i, _ in enumerate(model.layers[2].layers):
        model.layers[2].layers[i].set_weights(
            bp_model.layers[2].layers[i].get_weights()
        )
    return model
...
def make_mse_loss(inputs, outputs):
    ...
    outputs_eta = tf.concat(
        [
            outputs[:, 0:1, :],
            outputs_eta_egamma[:, 1:5, :],
            outputs_eta_muons[:, 5:9, :],
            outputs_eta_jets[:, 9:19, :],
        ],
        axis=1,
    )
    # use both tricks
    outputs = tf.concat(
        [outputs[:, :, 0], outputs_eta[:, :, 1], outputs_phi[:, :, 2]], axis=2
    )
    # mask zero features
...
class VAE(Model):
    def __init__(self, encoder, decoder, **kwargs):
        ...
        self.reconstruction_val_loss_tracker = metrics.Mean(
    ...
    def test_step(self, data):
        ...
        self.reconstruction_val_loss_tracker.update_state(
            (1 - beta) * reconstruction_loss
        )
...
def dense_vae():
    ...
    x = LeakyReLU(alpha=0.3)(x)
    dec = Dense(
        input_shape, kernel_initializer=tf.keras.initializers.HeUniform(seed=42)
    )(x)
...
class conv_vae(Model):
    def __init__(self, image_shape, latent_dim, beta=1.0, quant_size=0, **kwargs):
        ...
        z = Lambda(self.sample_z, output_shape=(latent_dim,), name="z")(
            [z_mean, z_log_var]
        )
...
def conv_ae(image_shape, latent_dim, quant_size=0, pruning="not_pruned"):
    ...
    input_encoder = Input(shape=image_shape[1:], name="encoder_input")
    if quant_size != 0:
        quantized_inputs = QActivation(f"quantized_bits(16,10,0,alpha=1)")(
            input_encoder
        )
        x = ZeroPadding2D(((1, 0), (0, 0)))(quantized_inputs)
    else:
    ...
    #
    x = (
        Conv2D(16, kernel_size=(3, 3), use_bias=False, padding="valid")(x)
        if quant_size == 0
        else QConv2D(
            16,
            kernel_size=(3, 3),
    ...
    #
    x = (
        Conv2D(32, kernel_size=(3, 1), use_bias=False, padding="same")(x)
        if quant_size == 0
        else QConv2D(
            32,
            kernel_size=(3, 1),
    ...
    enc = (
        ...
        if quant_size == 0
            ...
            latent_dim,
            kernel_quantizer=f"quantized_bits({quant_size},{int_size},0,alpha=1)",
            bias_quantizer=f"quantized_bits({quant_size},{int_size},0,alpha=1)",
        )(x)
    )
    ...
    x = (
        Conv2D(16, kernel_size=(3, 1), use_bias=False, padding="same")(x)
        if quant_size == 0
        else QConv2D(
            16,
            kernel_size=(3, 1),
    ...
    dec = (
        Conv2D(1, kernel_size=(3, 3), use_bias=False, padding="same")(x)
        if quant_size == 0
        else QConv2D(
            1,
            kernel_size=(3, 3),
            use_bias=False,
            padding="same",
            kernel_quantizer="quantized_bits(16,10,0,alpha=1)",
        )(x)
    )
    #
    decoder = Model(inputs=input_decoder, outputs=dec)
    ...
    if pruning == "pruned":
        ...
        pruning_schedule = tfmot.sparsity.keras.PolynomialDecay(
            ...
            final_sparsity=0.5,
            begin_step=start_pruning,
            end_step=end_pruning,
        )
        encoder_pruned = tfmot.sparsity.keras.prune_low_magnitude(
            encoder, pruning_schedule=pruning_schedule
        )
        encoder = encoder_pruned
        decoder_pruned = tfmot.sparsity.keras.prune_low_magnitude(
    ...
    # load weights
    if pruning == "pruned":
        autoencoder = model_set_weights(
            autoencoder, f"output/model-conv_ae-8-b0-q0-not_pruned", quant_size
        )
    # compile AE
    autoencoder.compile(optimizer=Adam(lr=3e-3, amsgrad=True), loss=make_mse)
    return autoencoder
